...
void GeneralRelativityWindow::Render(bool* p_open, UI* ui) {
    ...
    std::string latexMatrix = "$\\begin{pmatrix}\n";
    ...
    for (int row = 0; row < 4; ++row) {
        for (int col = 0; col < 4; ++col) {
            latexMatrix += matrix[row * 4 + col];
            if (col < 3) latexMatrix += " & ";
        }
        if (row < 3) latexMatrix += " \\\\\n";
        else latexMatrix += "\n";
    }
    latexMatrix += "\\end{pmatrix}$";

    const auto& tex = LatexRenderer::Instance().RenderLatex(latexMatrix, 150);
    if (tex.valid) {
        ImGui::Image((ImTextureID)(intptr_t)tex.textureID, ImVec2(tex.width, tex.height));
    } else {
        ImGui::TextWrapped("Failed to render LaTeX matrix.");
    }
