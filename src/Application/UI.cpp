...
void UI::RenderSidebar() {
    ...
    if (ImGui::Begin("##Sidebar", nullptr, flags)) {
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            if (m_sidebarHoverAnim[i] < targetAnim) {
                m_sidebarHoverAnim[i] = glm::min(m_sidebarHoverAnim[i] + delta, targetAnim);
            } else if (m_sidebarHoverAnim[i] > targetAnim) {
                m_sidebarHoverAnim[i] = glm::max(m_sidebarHoverAnim[i] - delta, targetAnim);
            }

            // Calculate colors with animation
            float animValue = m_sidebarHoverAnim[i];
        ...
        m_hoveredSidebarItem = hoveredItem;
    }
    ImGui::End();
